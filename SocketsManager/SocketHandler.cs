...
namespace ASP_Projekt_WebChat_PiotrBudz.SocketsManager
{
    public abstract class SocketHandler
    {
        ...
        public async Task SendMessage(WebSocket socket, string message)
        {
            ...
            var formattedMessage = separatorIndex != -1 ? message.Substring(separatorIndex + 1) : message;

            await socket.SendAsync(new ArraySegment<byte>(Encoding.ASCII.GetBytes(formattedMessage), 0, formattedMessage.Length),
